    ...
    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
      {tokens.map((token, index) => (
        <Link
          ...
          className="border border-zinc-800 p-4 hover:border-zinc-600 hover:bg-zinc-900/50 transition-all group text-center relative"
        >
          {index === 0 && (
            <span className="absolute -top-2 -right-2 bg-green-500 text-black text-[10px] font-bold px-2 py-0.5">
              NEW
            </span>
          )}
          {token.image ? (
            ...
            <img
              src={token.image}
              alt={token.name}
              className="w-12 h-12 object-cover bg-zinc-800 mx-auto mb-3"
            />
          ) : (
            <div className="w-12 h-12 bg-zinc-800 flex items-center justify-center mx-auto mb-3 text-lg font-bold text-zinc-600">
